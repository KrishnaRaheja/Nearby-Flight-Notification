...
from windows_toasts import Toast, InteractableWindowsToaster, ToastAudio, ToastDuration
...
paused = False
tokens_used = 0 # Track API calls in a session
...
try:
    while True:
        if paused:
            ...
            time.sleep(15) # reduce load on CPU constantly checking
        ...
        if data.get('states') is None:
                ...
                if plane_lat and plane_lon: # if either is None, skip, cannot find position anyway
                    ...
                    if dist_to_plane_km <= radius_km:
                        ...
                        if icao24 not in seen_aircraft:
                            ...
                            # Audio feature not working
                            #new_toast.audio = ToastAudio(Path('assets/system-notification-02-wav-format.wav'))
            ...
            # Status update
            if new_count > 0:
                print(f"[{new_count} new aircraft detected] [Tokens: {tokens_used}]")
            else:
                print(f"[Monitoring... {len(current_aircraft)} aircraft in range] [Tokens: {tokens_used}")
